...
export function loader ({params}){
        console.log(params)
        const hostDataVans = getHostVans(params.id);
        return {currentVan : hostDataVans}
}
  

  function hostVansDetailsDefer(currentVan) {
  ...
  return (
    <>
        ...
        <Suspense fallback={isLoading()}>
          <Await resolve={currentVan.currentVan} errorElement={<p className="text-red-500 text-center">Failed to load van details. Please try again later.</p>}>
            {hostVansDetailsDefer}
          </Await>
        </Suspense>
